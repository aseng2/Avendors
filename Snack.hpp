...
class Snack
{
  ...
  public:
   ...
   ~Snack           (                            )   noexcept;
    ...
    void upcCode    ( std::string newUpcCode     ) ;                // String parameters intentionally passed by value
    void brandName  ( std::string newBrandName   ) ;                // Modifiers available for l-values only         (The & at the end says these functions will be called only for l-values)
    void productName( std::string newProductName ) ;
    void vendingNum    ( std::string newVendingNum     ) ;
    void quantity (int newQuantity) ;                // OK:     GroceryItem b; b.price(13.99);        (b is an l-value, i.e. a named object)
    void price      ( double      newPrice       ) ; 
    void expireddate (int newExpiredDate);               // Error:  GroceryItem{}.price(13.99);           (The default constructed GrocerItem is an r-value, i.e., an unnamed temporary object)
  ...
  private:
    ...
    double      _price = 0.0;
    int _expireddate = 0;                                                 // the cost of the item in US Dollars (Ex:  2.29, 1.19)
};